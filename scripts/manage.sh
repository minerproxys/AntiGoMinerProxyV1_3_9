...
install(){
    ...
    sleep 2s
    echo "GoMinerProxy V1.3.9已经破解 安裝到/root/go_miner_proxy"
    cat /root/go_miner_proxy/pwd.txt
    echo ""
    echo "您可以使用指令screen -r go_miner_proxy查看网页端口和密码"
}
...
update(){
    ...
    echo "GoMinerProxy 已经更新至V1.3.9版本并，已破解、已启动"
    cat /root/go_miner_proxy/pwd.txt
    echo ""
    echo "您可以使用指令screen -r go_miner_proxy查看程序输出数据"
}
